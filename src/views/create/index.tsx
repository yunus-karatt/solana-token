...
export const CreateView: FC = ({ setOpenCreateModal }) => {
    ...
    if (file) {
      try {
        ...
        const response = await axios({
          ...
          data: formData,
          headers: {
            pinata_api_key: "41ffff80f288429e3c4c",
            pinata_secret_api_key:
              "272d14ebd1057c88437ef30b58a03c553f23a7e12b73c7c304aad7a159c27fd6",
            "Content-Type": "multipart/form-data",
          },
        });

        const ImagHash = `https://gateway.pinata.cloud/ipfs/${response.data.IpfsHash}`;
        return ImagHash;
      } catch (error: any) {
        notify({ type: "error", message: "Upload to pinnata image failed" });
      }
      setIsLoading(false);
    }
  };
  ...
  const uploadMetadata = async (token) => {
    ...
    if (!name || !symbol || !description || !image) {
      notify({ type: "error", message: "items missing" });
      return;
    }
  ...
  return (
    <>
      {isLoading && (
        <div className="absolute top-0 left-0 z-0 flex h-screen w-full items-center justify-center bg-black/[.3] backdrop-blur-[10px]">
          <ClipLoader />
        </div>
      )}
      {tokenMintAddress ? (
                ...
                <div className="lg:ps-0 flex flex-col p-10">
                  <div className="pb-6 my-auto">
                    ...
                    <p className="text-default-300 mb-8 max-w-sm">
                      Kindly provide all the detail about your token
                    </p>
